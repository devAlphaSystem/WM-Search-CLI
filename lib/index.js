...
export async function search(query, options = {}) {
  const { limit = DEFAULT_LIMIT, timeout = DEFAULT_TIMEOUT, sort, concurrency: rawConcurrency = DEFAULT_CONCURRENCY, state, make, model, minPrice, maxPrice, minYear, maxYear, minKm, maxKm, transmission, strict = false, noRateLimit = false, onFirstResponse = null, onErrorResponse = null } = options;
  ...
  if (onFirstResponse) {
    try {
      await onFirstResponse({ url: firstUrl, body: JSON.stringify(firstJson, null, 2), timestamp: new Date().toISOString() });
    } catch {}
  }
  ...
  if (!firstJson || !Array.isArray(firstJson.SearchResults)) {
    if (onErrorResponse) {
      try {
        await onErrorResponse({ url: firstUrl, body: JSON.stringify(firstJson, null, 2), error: "Could not extract search results", timestamp: new Date().toISOString() });
      } catch {}
    }
...
/**
 * Fetches the API URL, parses the JSON response, and returns the parsed object.
 *
